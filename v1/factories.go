...
func BrokerFactory(cnf *config.Config) (brokers.Interface, error) {
	...
	if strings.HasPrefix(cnf.Broker, "nats://") {
		return brokers.NewNATSBroker(cnf), nil
	}
...
func BackendFactory(cnf *config.Config) (backends.Interface, error) {
	...
	if strings.HasPrefix(cnf.ResultBackend, "memsqlcache://") {
		connString, err := ParseMemsqlConnString(cnf.ResultBackend)

		if err != nil {
			return nil, err
		}

		return backends.NewMemsqlCacheBackend(cnf, connString), nil
	}
...
// ParseMemsqlConnString ...
func ParseMemsqlConnString(url string) (connString string, err error) {
	parts := strings.Split(url, "memsqlcache://")

	if len(parts) != 2 {
		return "", fmt.Errorf(
			"Memsqlcache result backend connection string should be in format memsqlcache://host:port@tcp(user:password)/dbname, instead got %s",
			url,
		)
	}
